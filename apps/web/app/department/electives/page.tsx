        ...
        <ProtectedRoute allowedRoles={['DEPT_ADMIN']}>
            <DashboardLayout navItems={DEPT_ADMIN_NAV} title="Elective Baskets">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-text-primary">Elective Baskets</h2>
                        <p className="text-text-secondary dark:text-text-muted">Manage groups of elective subjects that must be scheduled in parallel.</p>
                    </div>
                    <Button onClick={() => {
                        setForm(JSON.parse(JSON.stringify(emptyForm)));
                        setError('');
                        setIsAddOpen(true);
                    }} className="bg-primary shadow-md hover:bg-primary/90 text-text-primary">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {filtered.map(basket => (
                            <Card key={basket.id} className="glass-card shadow-sm border-slate-200 dark:border-border-hover hover:shadow-md transition-shadow group">
                                <CardHeader className="pb-3 border-b border-slate-100 dark:border-border">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <CardTitle className="text-lg font-bold text-slate-800 dark:text-text-primary">{basket.name}</CardTitle>
                                        ...
                                        <div className="flex gap-1">
                                            <Button variant="ghost" size="icon" onClick={() => {
                                                ...
                                                setError('');
                                                setIsEditOpen(true);
                                            }} className="h-8 w-8 text-text-muted hover:text-indigo-600 dark:hover:text-indigo-400">
                                                <LuPencil className="w-4 h-4" />
                                            </Button>
                                            <Button variant="ghost" size="icon" onClick={() => handleDelete(basket.id)} className="h-8 w-8 text-text-muted hover:text-red-600">
                                ...
                                <CardContent className="pt-4">
                                    <div className="flex gap-4 mb-4 text-xs font-bold uppercase text-text-muted dark:text-text-secondary">
                                    ...
                                    <div className="space-y-2">
                                        {basket.options.map((opt, idx) => (
                                            <div key={idx} className="flex flex-col p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-border transition-colors hover:border-indigo-200 dark:hover:border-indigo-500/30">
                                                <div className="flex items-center justify-between text-sm">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="text-[10px] text-text-secondary dark:text-text-muted flex items-center gap-1.5 mt-0.5">
                                                    ...
                                                    <div className="flex flex-col items-end gap-1">
                                                        <div className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white dark:bg-slate-800 border dark:border-border-hover text-slate-600 dark:text-text-muted shadow-sm">
                                                ...
                                                {opt.subgroups && opt.subgroups.length > 0 && (
                                                    <div className="mt-3 pl-13 pr-1 space-y-2">
                                                        <div className="text-[10px] font-bold uppercase text-text-muted flex items-center gap-2">
                                                            <span>Capacity Split ({opt.subgroups.length} Grp)</span>
                                                            <div className="h-px bg-slate-200 dark:bg-surface-hover flex-1"></div>
                                                        </div>
                                                        <div className="grid grid-cols-2 gap-2">
                                                            {opt.subgroups.map((sg: any, sgIdx: number) => (
                                                                <div key={sgIdx} className="flex items-center justify-between bg-white dark:bg-slate-800/80 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-border-hover text-[11px] shadow-sm">
                                                                    <span className="font-medium text-slate-600 dark:text-text-muted">{sg.name || `Subgroup ${sgIdx + 1}`}</span>
                                                                    <span className="font-bold text-indigo-600 dark:text-indigo-400">{sg.enrollmentCount} <span className="text-[9px] font-medium text-text-muted">capacity</span></span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                        ...
                        {filtered.length === 0 && (
                            <div className="col-span-full py-16 text-center text-text-secondary dark:text-text-muted glass-card rounded-xl border-dashed border-2 border-slate-200 dark:border-border-hover">
                                <LuLayers className="w-12 h-12 text-text-muted dark:text-slate-600 mx-auto mb-3" />
                ...
                <Dialog open={isAddOpen || isEditOpen} onOpenChange={(val) => { if (!val) { setIsAddOpen(false); setIsEditOpen(false); } }}>
                    <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto glass-card dark:border-border-hover">
                        <DialogHeader>
                            <DialogTitle className="dark:text-text-primary uppercase tracking-wider text-xs font-bold text-indigo-500">
                        ...
                        <div className="space-y-6 py-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-tight text-text-secondary dark:text-text-muted">Basket / Group Name</label>
                                    <Input
                                        placeholder="e.g. Elective-III Specialization"
                                        value={form.name}
                                        onChange={e => setForm({ ...form, name: e.target.value })}
                                        className="dark:bg-slate-900/50 dark:border-border-hover dark:text-text-primary font-medium"
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-tight text-text-secondary dark:text-text-muted">Subject / Basket Code</label>
                                    <Input
                                        placeholder="e.g. CS-404-B"
                                        value={form.subjectCode}
                                        onChange={e => setForm({ ...form, subjectCode: e.target.value })}
                                        className="dark:bg-slate-900/50 dark:border-border-hover dark:text-text-primary font-mono"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-tight text-text-secondary dark:text-text-muted">Program</label>
                                    <select
                                        className="w-full h-10 rounded-md border border-input bg-background dark:bg-slate-900/50 dark:border-border-hover dark:text-text-primary px-3 py-2 text-sm outline-none font-medium"
                                        value={form.program}
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-tight text-text-secondary dark:text-text-muted">Semester</label>
                                    <select
                                        className="w-full h-10 rounded-md border border-input bg-background dark:bg-slate-900/50 dark:border-border-hover dark:text-text-primary px-3 py-2 text-sm outline-none font-medium"
                                        value={form.semester}
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-tight text-text-secondary dark:text-text-muted">Weekly Hours</label>
                                    <Input
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center px-1">
                                    <label className="text-xs font-bold uppercase tracking-tight text-text-secondary dark:text-text-muted flex items-center gap-2">
                                        <LuLayers className="w-4 h-4 text-primary" /> Basket Options
                                    </label>
                                    <Button type="button" variant="outline" size="sm" onClick={handleAddOption} className="h-7 text-[10px] font-bold uppercase tracking-wider dark:border-border-hover dark:text-text-muted">
                                ...
                                <div className="space-y-3">
                                    {form.options.map((opt: any, idx: number) => {
                                        ...
                                        return (
                                            <div key={idx} className="p-4 rounded-xl border border-slate-100 dark:border-border bg-slate-50/50 dark:bg-slate-900/30 relative">
                                                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                                                    <div className="md:col-span-5 space-y-2">
                                                        <label className="text-[10px] font-bold uppercase tracking-tight text-text-muted dark:text-text-secondary">Course / Specialization</label>
                                                        <div className="relative group/search">
                                                            <select
                                                                className="w-full h-9 rounded-md border border-input bg-background dark:bg-slate-900/50 dark:border-indigo-500/20 dark:text-text-primary px-3 text-sm outline-none font-bold appearance-none cursor-pointer"
                                                                value={opt.courseId}
                                                    ...
                                                    <div className="md:col-span-4 space-y-2">
                                                        <label className="text-[10px] font-bold uppercase tracking-tight text-text-muted dark:text-text-secondary">Assigned Faculty</label>
                                                        <select
                                                            className="w-full h-9 rounded-md border border-input bg-background dark:bg-slate-900/50 dark:border-indigo-500/20 dark:text-text-primary px-3 text-sm outline-none"
                                                            value={opt.facultyId || ''}
                                                    ...
                                                    <div className="md:col-span-2 space-y-2">
                                                        <label className="text-[10px] font-bold uppercase tracking-tight text-text-muted dark:text-text-secondary">Enrollment</label>
                                                        <Input
                                                            type="number" min="1"
                                                            value={opt.enrollmentCount}
                                                            onChange={e => handleOptionChange(idx, 'enrollmentCount', parseInt(e.target.value) || 0)}
                                                            className="h-9 dark:bg-slate-900/50 dark:border-indigo-500/20 dark:text-text-primary font-bold text-center"
                                                        />
                                                    </div>
                                                    <div className="md:col-span-1 flex justify-center pb-1">
                                                        <Button variant="ghost" size="icon" onClick={() => handleRemoveOption(idx)} className="h-8 w-8 text-text-muted hover:text-red-500 dark:hover:text-red-400">
                                                            <LuTrash2 className="w-4 h-4" />
                                                        </Button>
                                                    </div>
                                                </div>
                        ...
                        <DialogFooter className="border-t dark:border-border pt-6 mt-2">
                            <Button variant="outline" className="dark:border-border-hover dark:text-text-muted dark:hover:bg-slate-800" onClick={() => { setIsAddOpen(false); setIsEditOpen(false); }}>Cancel</Button>
                            <Button className="bg-primary hover:bg-primary/90 text-text-primary min-w-[120px] font-bold" onClick={handleSave}>
