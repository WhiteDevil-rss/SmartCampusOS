...
export default function UniAdmissionsPage() {
    ...
    const fetchApplications = async () => {
        try {
            setLoading(true);
            const res = await api.get(`/v2/admissions/admin?universityId=${user?.universityId}`);
        ...
        <DashboardLayout navItems={[]} title="Admission Requests">
            <div className="space-y-6">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-2xl font-bold font-space-grotesk text-white">University Applicants</h2>
                        <p className="text-text-secondary text-sm">Review incoming admission payloads.</p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-12 glass-morphism rounded-2xl border border-border">
                        ...
                        <p className="text-text-secondary text-center">There are currently no pending admission applications in your university portal.</p>
                    </div>
                ) : (
